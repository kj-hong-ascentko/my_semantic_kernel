...
class KernelConfig:
    ...
    def add_text_backend(
        self, 
        service_id: str, 
        backend: Union[TextCompletionClientBase, Callable[["KernelBase"], TextCompletionClientBase]],
        overwrite: bool = False        
    ) -> "KernelConfig":
        if not service_id:
            raise ValueError("service_id must be a non-empty string")
        if not overwrite and service_id in self._text_backends:
            raise ValueError(f"Text backend with service_id '{service_id}' already exists")
    ...
    def set_default_text_backend(self, service_id: str) -> "KernelConfig":
        if service_id not in self._text_backends:
            raise ValueError(
                f"Text backend with service_id '{service_id}' not found"
            )
        self._default_text_backend = service_id
        return self
